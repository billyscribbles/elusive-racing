...
- [x] ✅ **C3. BACS bank details** — DONE
  All three hardcoded copies consolidated into `data/bacs-config.json`. Consumers: `src/pages/PaymentPage.jsx` (checkout BACS form), `src/pages/OrderConfirmationPage.jsx` (on-screen confirmation), `src/lib/generateReceiptPdf.js` (PDF download). Real CBA details now in place: BSB `063-171`, Account `1121 6176`, Account Name `Elusive Racing Pty Ltd`.
...
- [x] ✅ **H12. Build-time secret scan** — DONE
  `scripts/scan-secrets.mjs` runs as a postbuild step (`"build": "vite build && node scripts/build-sitemap.mjs && node scripts/scan-secrets.mjs"`). Scans the `dist/` text bundle for WC consumer keys (`ck_`/`cs_`), Stripe secret keys (`sk_live_*`/`sk_test_*`), and Stripe restricted keys (`rk_*`). Build fails if any pattern matches. Publishable Stripe key (`pk_*`) and WC URL are intentionally not flagged.
...
- [x] ✅ **M5. 420px breakpoint check** — DONE
  Static CSS audit + live browser walk (Chrome DevTools MCP emulating 420x900 mobile) across home, shop, PDP, brands, services, about, contact, wholesale-registration, returns, terms, checkout, my-account flows — every route reported `document.body.scrollWidth === window.innerWidth === 420`, i.e. zero horizontal overflow. The ≤480px breakpoint used throughout the codebase correctly covers 420px (cp-row-3 / pp-layout / co-step-text all collapse). One real bug found and fixed: `CartIcon.css .cart-drawer` was hard-coded at `width: 360px`, leaving only 15px of backdrop at 375px — now `width: 100%; max-width: calc(100vw - 32px)` at ≤480px so the tap-to-close strip stays usable.
- [x] ✅ **M6. Reduced-motion** — DONE
  `Hero.jsx` reads `prefers-reduced-motion: reduce` and skips mounting the `<video>` elements entirely (poster image shown instead). Global CSS rule in `App.css` now collapses every `animation-duration` / `transition-duration` site-wide to `0.01ms` (preserving `animationend`/`transitionend` events) when the user has reduced-motion on — covers ChatWidget slide/bounce, CartIcon fade/slide, FeaturedProducts/ShopPage/ProductPage shimmer, PromoBanner pulse, Navigation dropdown/slide, all spinners, etc. Note: `BrandsSection` is now a static grid (no marquee) so there's nothing to disable there.
...
- [x] ✅ **L5.** Wholesale form already uses proper `<label htmlFor="...">` + `<input id="...">` pairs for all 14 fields in `src/pages/WholesalePage.jsx:232+`. Placeholders are supplementary only. No work needed.
