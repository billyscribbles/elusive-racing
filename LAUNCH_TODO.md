...
- [x] ⏳ **C3. BACS bank details** — CODE DONE, REAL ACCOUNT NUMBER STILL NEEDED
  Extracted all three hardcoded copies into one file: `data/bacs-config.json`. Consumers: `src/pages/PaymentPage.jsx` (checkout BACS form), `src/pages/OrderConfirmationPage.jsx` (on-screen confirmation), `src/lib/generateReceiptPdf.js` (PDF download). Bank/account name/BSB/reference are already the real Elusive Racing values; **Account No. still reads `"1234 5678"`** as a placeholder.
  **Still need from you:** edit `data/bacs-config.json` and drop in the real CBA account number, then rebuild. No code change required.
